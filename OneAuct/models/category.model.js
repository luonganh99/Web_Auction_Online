...
module.exports = {
    all: () =>  db.load('select CatName as ParentName, CatID from categories where ParentID is null'),
    //allSubs: () =>  db.load('select * from categories where ParentID is not null'),
    ...
    single: async (catID) => {
        const rows = await db.load(`select * from categories where CatID = ${catID}`);
        return rows[0];
    },
