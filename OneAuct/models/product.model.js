...
module.exports = {
    ...
    joinedlist: (userID) => db.load(`select * from (select ProID, Price from users_bid_products where UserID = ${userID} and State = 1 order by BidID desc limit 1) b , products p where b.ProID = p.ProID and p.State = 1`),
    wonlist: (userID) => db.load(`select uSeller.FullName, p.ProID, p.ProName, p.ProInfo, p.ExpiryDate, p.CurrentPrice FROM (select ProID, Price from users_bid_products where UserID =  ${userID}  and State = 1 order by BidID desc limit 1) b , products p, users uSeller, users uBidder  where b.ProID = p.ProID and p.SellerID = uSeller.UserID and uBidder.UserID = p.BidderID and p.State = 1 and uBidder.UserID =  ${userID}`)
}